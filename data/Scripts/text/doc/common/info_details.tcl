...
proc di_render_subwindow_main_gnome {} {
	...
	println_kv [lmsg Name] [get_objname $di_id]
	println_kv [lmsg Gender] [lmsg [get_objgender $di_id]]
	set exp_level [call_method $di_id get_exp_level]
	println_kv [lmsg Level] "$exp_level"
	set exp_points [call_method $di_id get_exp_points]
	set exp_points_ftnl [call_method $di_id get_exp_points_ftnl]
	println_kv [lmsg Experience] "$exp_points / $exp_points_ftnl"
	set exp_skill_points [call_method $di_id get_exp_skill_points]
	println_kv [lmsg SkillPoints] "$exp_skill_points"
	foreach key [db_gnome_skills_keys] {
		if { ![call_method $di_id has_acquired_skill $key] } {
			println [btn [lmsg $key] "call_method $di_id acquire_skill $key; di_reload_layout" [call_method $di_id can_acquire_skill $key]]
		}
	}
	layout print "/p"
...
proc di_render_subwindow_cheats_gnome {} {
	...
	foreach attr {atr_ExpMax exp_Nahrung exp_Holz exp_Stein exp_Metall exp_Transport exp_Energie exp_Service exp_F_Sword exp_F_Twohanded exp_F_Defense exp_F_Ballistic exp_F_Kungfu} {
		println_kargs $attr [btn -10 "add_attrib $id $attr -0.1; di_reload_layout"] [btn -5 "add_attrib $id $attr -0.05; di_reload_layout"] [btn -1 "add_attrib $id $attr -0.01; di_reload_layout"] [btn 0 "set_attrib $id $attr 0.0; di_reload_layout"] [btn +1 "add_attrib $id $attr 0.01; di_reload_layout"] [btn +5 "add_attrib $id $attr 0.05; di_reload_layout"] [btn +10 "add_attrib $id $attr 0.1; di_reload_layout"]
	}
