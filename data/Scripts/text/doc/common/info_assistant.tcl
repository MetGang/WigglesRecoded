...
proc pi_reload_layout {} {
	global pi_tab pi_buildings_tab pi_sflag_age pi_sflag_gender pi_sflag_working pi_aflag_rarity pi_aflag_status
	layout reload
}
...
proc pi_set_aflag_rarity {flag} {
	global pi_aflag_rarity
	set pi_aflag_rarity $flag
	pi_reload_layout
}

proc pi_set_aflag_status {flag} {
	global pi_aflag_status
	set pi_aflag_status $flag
	pi_reload_layout
}

proc pi_btn_set_aflag_rarity {flag lkey} {
	global pi_aflag_rarity
	return [btn [lmsg $lkey] "pi_set_aflag_rarity $flag" [expr {$pi_aflag_rarity != $flag}]]
}

proc pi_btn_set_aflag_status {flag lkey} {
	global pi_aflag_status
	return [btn [lmsg $lkey] "pi_set_aflag_status $flag" [expr {$pi_aflag_status != $flag}]]
}
...
proc pi_render_subwindow_achievements {} {
	...
	if { $pi_aflag_status == "all" } {
		...
		layout print "/(fn1)[lmsg Progress]: /(fn0)$unlocked_count / $total_count"
		layout print "/p"
	} else {
		if { $pi_aflag_status == "unlocked" } {
			set unlocked_count [call_method $aman get_unlocked_achv_count $pi_aflag_rarity]
			layout print "/(fn1)[lmsg Unlocked]: /(fn0)$unlocked_count"
		} elseif { $pi_aflag_status == "locked" } {
			set locked_count [call_method $aman get_locked_achv_count $pi_aflag_rarity]
			layout print "/(fn1)[lmsg Locked]: /(fn0)$locked_count"
		}
		layout print "/p"
	}
	...
	foreach key [call_method $aman get_achv_list $pi_aflag_rarity] {
		...
		layout print [btn "$title" "call_method $twman open info_achievement $key" [call_method $aman is_achv_unlocked $key]]
		layout print "/p"
		# layout print "/(fn1,bo6)[lmsg AchvTitle_$key]/p"
...
proc pi_render_subwindow_cheats {} {
	set cman [call_method_static CheatManager get_instance]
	set cheat_list [call_method $cman get_available_cheats]

	layout print "/(ml10,ta10,bo4)"

	println [btn Toggle "call_method $cman toggle_enabled; pi_reload_layout"]

	if { [call_method $cman is_enabled] } {
		println $cman
		foreach key $cheat_list {
			println_kv $key [btn [call_method $cman is_cheat_enabled $key] "call_method $cman toggle_cheat_enabled $key; pi_reload_layout"]
		}
	}
