...
def_class AchievementManager none none 0 {} {
	...
	obj_init {
		...
		foreach key [db_achievements_keys] {
			...
			# [6] = entry idx
			# [7] = entries
			set achv_$key [list 0 [db_achievements_get_steps $key] [db_achievements_get_rarity $key] 0 [db_achievements_get_type $key] [db_achievements_get_time_window $key] 0 [db_achievements_get_def_entries $key]]
		}
		...
		proc incr_achv_progress {key} {
			...
			switch $type {
				...
				"time" {
					...
					if { [lindex $entries $ref_entry_idx] != 0 } {
						if { [lindex $entries $entry_idx] - [lindex $entries $ref_entry_idx] < $time_window } {
							incr dividend
							set achv [lreplace $achv 0 0 $dividend]
							set achv [lreplace $achv 7 7 [db_achievements_get_def_entries $key]]
						}
					}
				}
			}
