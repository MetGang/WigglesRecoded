...
//		kill_all_ghosts
	...
	proc evt_zwerg_attribupdate_proc {} {
		...
		# Extinguish gnome when under water
		if { $is_burning && [isunderwater [vector_add [get_pos this] {0 -0.3 0}]] } {
			timer_event this evt_burnend -repeat 1 -interval 1 -userid 0 -attime [expr {[gettime] + 0.1}]
		}

		# Deal damage when gnome is on fire
		if { $is_burning } {
			add_attrib this atr_Hitpoints -0.1
		}

		# Is gnome wearing a diving bell? Well yes, but actually no.
		if { $is_wearing_divingbell && [inv_find this Taucherglocke] == -1 } {
		...
		# Gnome under 1 hitpoint dies
		if { [get_attrib this atr_Hitpoints] < 0.01 } {
			set_event this evt_zwerg_die -target this
			log EVENT "[get_objname this] sent to death"
			return
		}

		# Attribute variables initialization
		...
		# Facial animations based on attributes
		...
		# Loss of hitpoints and mood due to hunger and fatigue
		if { $at_Nu < 0.2 } {
		...
		# Attributes decrease faster while working
		...
		# Attribute loss due to civ state (overall disappointment caused by several factors)
		fincr sub_Mo $spt_disappointment

		# Decrease attributes only when gnome is not trapped
		if { $trap_mode == 0 } {
			# Mood decreases slower when gnome has certain "fun" items in inventory
			if { $spt_fun_items } {
				set sub_Mo [expr {$sub_Mo * 0.5}]
			}

			# AI controlled gnomes play on easy mode - skill issue
			if { ![im_a_human] } {
		...
		if { [get_attrib this atr_Nutrition] < 0.2 } {
			...
			if { $ntNutrMessage != -1 } {
				newsticker delete $ntNutrMessage
				set ntNutrMessage -1
			}
		}

		# Low hitpoints sticker handling
		if { [get_attrib this atr_Hitpoints] < 0.5 } {
			...
			if { $ntHitMessage != -1 } {
				newsticker delete $ntHitMessage
				set ntHitMessage -1
			}
		}

		# set_objicon this -1 1 <id> <seconds>
		# data/Texture/Misc/events.tga

		# Red hunger (mushroom) icon
		if { $at_Nu + $sub_Nu < 0.3 } { set_objicon this -1 1 6 5 }
		# Red fatique (bed) icon
