...
/// Rausgenommen aus den �berlasteten z_work und z_procs
...
// L�st eine Walk-Action auf dem aktuelle Zwerg aus
// !! niemand sollte das von Hand tun, falls es nicht unbedingt n�tig ist !!
//
// syntax:	walk_action "paramstring" "finishcode optional" "breakcode optional" "erweiterte Parameter"
//
// paramstring wird direkt an die action �bergeben
...
proc walk_action {paramstring {finishcode ""} {breakcode ""} args} {
	...
	// zus�tzliche W�nsche des Skripts ber�cksichtigen
	...
	if {[get_gnomeposition this] == 0} {
		...
		if {[inv_find this Kristallstrahl]==-1||[lindex $digpos 1]>[get_posy this]} {
			...
			if {$height>1.6} {
				...
				set accident 0								;# keine Unfallanimation f�r vorne da
			} else {
...
proc walk_pos {pos {force 0}} {
	...
	note_pathlength $pos
	state_disable this
//	log "[get_objname this]: state_disable f�r walkpos..."
...
// l�uft in die N�he eines Items (oder eines Punktes)
