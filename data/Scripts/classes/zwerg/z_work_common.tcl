...
// verlegt ein Item aus dem Inventar des Zwerges in das der Produktionsst�tte
...
// bringt einen einzelnen Gegenstand (aus dem Inventar) zur Produktionsst�tte,
// d.h. sucht einen Platz zum Hinlegen, l�uft hin und legt den Gegenstand ab
// (und transportiert ihn ins Inventar der Produktionsst�tte via prod_transfertoprod)
...
//	log "PROGD_BUILDUP: this = [get_ref this], item = $item"
    ...
    set_roty $item 0
	# Schnellaufbau f�r Items ohne Aufbauanimation
...
proc conquer {building} {
	...
	//einige Produktionsst�tten haben buildup_step = 0 !!!!
...
proc conquer_callback {building offset} {
	//die Zeit soll von building_step abh�ngig sein
	...
	if {[get_owner $building] != [get_owner this]} {
		...
		if {[get_attrib $building nt_Message] == 1} {
		 ...
		 if {$strength < 0.9} {
			...
			set_prod_ownerstrength $building 1.0 ;# damit strengt nicht gr��er als 1.0 wird
...
// wechselt die M�tze des Zwerges
...
proc prod_change_muetze {category {auf_ab both} {animopt "nothing"}} {
	...
	if {$is_wearing_divingbell != 0} {
		return true													;// Taucherglocke verhindert alle M�tzenaktionen
	}
...
// erzeugt eine M�tze und linkt sie an den Kopf; kann nach einer entsprechenden
// Anim in die Tasklist geh�ngt werden
...
// kann nach einer entsprechenden Anim in die Tasklist geh�ngt werden
